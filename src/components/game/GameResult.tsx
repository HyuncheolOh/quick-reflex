...
export const GameResult: React.FC<GameResultProps> = ({
  ...
  const renderStatCard = (
    ...
    <Card
      ...
      variant="elevated"
    >
      <Text style={[styles.statTitle, { color: colors.TEXT_SECONDARY }]}>
        {title}
      </Text>
      <Text style={[styles.statValue, { color: color || colors.TEXT_PRIMARY }]}>
        {value}
      </Text>
      {subtitle && (
        <Text style={[styles.statSubtitle, { color: colors.TEXT_TERTIARY }]}>
          {subtitle}
        </Text>
      )}
    </Card>
      ...
      <View style={styles.attemptsList}>
        {attempts.map((attempt, index) => (
          ...
          >
            ...
            >
              <Text
                style={[styles.attemptNumberText, { color: colors.TEXT_WHITE }]}
              >
                {index + 1}
              </Text>
            </View>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor: attempt.isValid
                    ? colors.SUCCESS
                    : colors.ERROR,
                },
              ]}
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <Card
        style={
          [
            ...
            { borderColor: performance.color, backgroundColor: colors.CARD },
          ] as any
        }
        variant="outlined"
      >
        ...
        <Text
          style={[styles.performanceAverage, { color: colors.TEXT_SECONDARY }]}
        >
          {t.results.averageTime}:{" "}
          {statistics.validAttempts > 0
            ? GameLogic.formatTime(statistics.averageTime)
            : "-"}
        </Text>
      </Card>
      ...
      <View style={styles.statsGrid}>
        {renderStatCard(
          t.results.bestTime,
          statistics.validAttempts > 0
            ? GameLogic.formatTime(statistics.bestTime)
            : "-",
        ...
        {statistics.worstTime > 0 &&
          statistics.validAttempts > 1 &&
          renderStatCard(
            t.results.worstTime,
